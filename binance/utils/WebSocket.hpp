...
typedef std::function<void(const string &data)> _OnError;
typedef std::function<void(const string &data)> _OnMsg;
                                ...
                                size_t len)
            {
                ...
                case LWS_CALLBACK_CLIENT_WRITEABLE:
                {
                    ...
                    for (auto item : p_cls_obj->data_buf)
                    {
                        ...
                        int n = lws_snprintf((char *)buf + LWS_PRE, WS_SEND_BUF_SIZE, "%s", item.c_str());
